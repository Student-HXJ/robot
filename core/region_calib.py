"""区域框选校准公共逻辑（供 calibrate_detect / calibrate_hpmp 共用）。

两个校准脚本的分工：
    - ``calibrate_detect.py``：只框选「检测框」（[detect] detect_region）
    - ``calibrate_hpmp.py``  ：只框选「HP/MP 血条框」
      （[hpmp] hp_bar_region / mp_bar_region）

两者流程完全一致（切游戏窗口 → 抓全屏 → 鼠标拖拽框选 → 存图校验 → 写回
config.toml），差别只在「要框哪几个区域」以及「存图校验方式」，因此公共流程
集中在本模块，避免在多个入口脚本里重复实现。

本模块不含任何按键/机器人逻辑，可被任意校准脚本复用。
"""

import re
import time

import cv2
import mss
import numpy as np

import config
from core.hpmp_monitor import calc_bar_percent, save_bar_debug
from core.utils import project_path
from core.win_window import activate_game_window

# 校准 key → 中文标签（控制台打印 / matplotlib 标题用中文时部分环境会渲染成
# 方框或乱码，故标题另用 EN_LABELS 的英文标签）
LABELS = {
    "detect": "检测区域",
    "hp": "HP 血条",
    "mp": "MP 血条",
}

# 校准 key → matplotlib 窗口标题用的英文标签
EN_LABELS = {
    "detect": "detection region",
    "hp": "HP bar",
    "mp": "MP bar",
}

# 校准 key → config.toml 中的变量名（用于打印推荐结果）
OUTPUT_NAMES = {
    "detect": "DETECT_REGION",
    "hp": "HP_BAR_REGION",
    "mp": "MP_BAR_REGION",
}

# 校准 key → 全屏总览上画框的颜色（BGR）
OVERVIEW_COLORS = {
    "detect": (0, 255, 255),  # 黄
    "hp": (0, 0, 255),        # 红
    "mp": (255, 0, 0),        # 蓝
}

# 最小框选尺寸（像素），小于此值视为误触
MIN_REGION_SIZE = 5


# ---------------------------------------------------------------------- #
#  抓图
# ---------------------------------------------------------------------- #

def switch_to_game():
    """抓全屏前把游戏窗口切到前台（全屏时避免抓到桌面/被遮挡窗口）。

    关键字取自 config.GAME_WINDOW_KEYWORD（与 screencap 一致）。
    切换失败静默继续（仍会抓屏，只是可能抓到桌面）。
    """
    try:
        keyword = config.GAME_WINDOW_KEYWORD
        if activate_game_window(keyword):
            print(f"[INFO] 已切换到游戏窗口（关键字={keyword}）")
        else:
            print(f"[WARN] 未找到游戏窗口（关键字={keyword}），可能抓到桌面")
        time.sleep(0.5)  # 等窗口真正置前再抓屏
    except Exception as e:
        print(f"[WARN] 切换游戏窗口失败（{e}），继续抓屏")


def grab_fullscreen():
    """抓取全屏并返回 BGR numpy 数组（不落盘）。"""
    with mss.mss() as sct:
        mon = sct.monitors[0]  # 全屏
        shot = sct.grab(mon)
        img = np.array(shot)[:, :, :3]
        img = np.ascontiguousarray(img)
    return img  # BGR


def save_fullscreen(full_img):
    """保存全屏截图（debug_fullscreen.png），便于事后核对区域位置。"""
    path = project_path("debug_fullscreen.png")
    cv2.imwrite(path, full_img)
    print(f"[INFO] 全屏已保存: {path}（可打开查看区域位置）")
    return path


# ---------------------------------------------------------------------- #
#  框选
# ---------------------------------------------------------------------- #

def try_gui_select(full_img, keys):
    """尝试用 matplotlib 交互框选；成功返回 {key: region}，失败返回 None。

    Args:
        full_img: 全屏 BGR 图
        keys: 要框选的校准 key 列表（如 ("detect",) 或 ("hp", "mp")）

    Returns:
        dict | None: {key: (left, top, w, h)}；无 GUI 环境时返回 None
    """
    try:
        import matplotlib
        matplotlib.use("TkAgg")
        import matplotlib.pyplot as plt
        from matplotlib.widgets import RectangleSelector
    except Exception as e:
        print(f"[WARN] 无法加载 GUI 框选（{e}），改用坐标输入方式")
        return None

    regions = {}

    for key in keys:
        label = LABELS[key]
        fig, ax = plt.subplots(figsize=(12, 7))
        # BGR -> RGB 显示
        ax.imshow(cv2.cvtColor(full_img, cv2.COLOR_BGR2RGB))
        ax.set_title(f"Drag to select the {EN_LABELS[key]} region, "
                     f"then close the window (or press Enter)")
        rect = {}

        def on_select(eclick, erelease):
            x1, y1 = int(round(eclick.xdata)), int(round(eclick.ydata))
            x2, y2 = int(round(erelease.xdata)), int(round(erelease.ydata))
            left = min(x1, x2)
            top = min(y1, y2)
            w = abs(x2 - x1)
            h = abs(y2 - y1)
            rect["region"] = (left, top, w, h)
            print(f"  {label} 区域: left={left} top={top} w={w} h={h}")

        RectangleSelector(ax,
                          on_select,
                          useblit=True,
                          button=[1],
                          minspanx=MIN_REGION_SIZE,
                          minspany=MIN_REGION_SIZE,
                          spancoords="pixels",
                          interactive=True)

        def on_key(event):
            if event.key == "enter":
                plt.close(fig)

        fig.canvas.mpl_connect("key_press_event", on_key)
        plt.show()

        if "region" not in rect:
            print(f"[WARN] 未框选 {label}，跳过")
            continue
        left, top, w, h = rect["region"]
        if w < MIN_REGION_SIZE or h < MIN_REGION_SIZE:
            print(f"[WARN] {label} 区域太小，跳过")
            continue
        regions[key] = (left, top, w, h)

    return regions


def manual_select(full_img, keys):
    """无 GUI 时，让用户根据屏幕坐标手动输入区域。"""
    h, w = full_img.shape[:2]
    print(f"[INFO] 全屏尺寸: {w} x {h}")
    print("请在游戏里把区域位置记下来，或直接输入屏幕坐标（左上角原点）。")
    regions = {}
    for key in keys:
        raw = input(f"输入 {LABELS[key]} 区域 (left top width height)，留空跳过: ").strip()
        if not raw:
            continue
        try:
            left, top, ww, hh = (int(x) for x in raw.split())
        except ValueError:
            print(f"[WARN] {LABELS[key]} 输入格式错误，跳过")
            continue
        left = max(0, left)
        top = max(0, top)
        ww = min(ww, w - left)
        hh = min(hh, h - top)
        if ww < MIN_REGION_SIZE or hh < MIN_REGION_SIZE:
            print(f"[WARN] {LABELS[key]} 区域太小，跳过")
            continue
        regions[key] = (left, top, ww, hh)
    return regions


# ---------------------------------------------------------------------- #
#  存图校验
# ---------------------------------------------------------------------- #

def verify_regions(full_img, regions, overview_name):
    """按框选结果存图，供人工核对识别是否准确。

    - detect: 保存原始裁剪图 debug_detect.png
    - hp/mp : 用 config.toml 的 HSV 颜色阈值计算百分比，保存原始图 +
      识别掩膜叠加图（debug_hp.png / debug_hp_mask.png 等）
    - 最后在全屏上画出所有框，保存 overview_name（如
      debug_regions_overview.png）供核对框是否对准

    Args:
        full_img: 全屏 BGR 图
        regions: {key: (left, top, w, h)}
        overview_name: 全屏总览图片文件名（相对项目根目录）
    """
    print()
    print("[INFO] 正在按框选结果存图校验识别 ...")

    overview = full_img.copy()
    for key, (left, top, w, h) in regions.items():
        crop = full_img[top:top + h, left:left + w]
        color = OVERVIEW_COLORS[key]
        if key == "detect":
            cv2.imwrite(project_path("debug_detect.png"), crop)
            print("[INFO] 已保存 debug_detect.png（检测区域裁剪图）")
        else:
            pct = calc_bar_percent(crop, key)
            save_bar_debug(crop, key, pct)
            print(f"[INFO] {key.upper()} 识别 = {pct:.1f}%"
                  f"（见 debug_{key}.png / debug_{key}_mask.png）")
            if pct <= 0:
                print(f"[WARN] {key.upper()} 未识别到血条颜色，框选可能没对准，"
                      "请重新框选")
        # 全屏总览上画框
        cv2.rectangle(overview, (left, top), (left + w, top + h), color, 2)
        cv2.putText(overview, key.upper(), (left + 3, max(10, top - 5)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

    cv2.imwrite(project_path(overview_name), overview)
    print(f"[INFO] 已保存 {overview_name}（全屏总览，核对框是否对准）")


# ---------------------------------------------------------------------- #
#  写回 config.toml
# ---------------------------------------------------------------------- #

def write_to_config(regions):
    """把框选结果写回 config.toml 的 [detect]/[hpmp] 节。

    只替换对应 key 的值（detect_region / hp_bar_region / mp_bar_region），
    保留文件里的中文注释与其他配置。找不到 key 或读写失败时打印警告并跳过，
    不阻塞校准流程。

    Args:
        regions: {key: (left, top, w, h)}

    Returns:
        list: 成功写入的 toml key 列表（如 ['detect_region', 'hp_bar_region']）
    """
    path = config.CONFIG_PATH
    try:
        with open(path, "r", encoding="utf-8") as f:
            text = f.read()
    except OSError as e:
        print(f"[WARN] 读取 {path} 失败（{e}），未写入")
        return []

    updated = []
    for key, (left, top, w, h) in regions.items():
        toml_key = OUTPUT_NAMES[key].lower()  # DETECT_REGION -> detect_region
        # 只匹配 "key = [旧值]"，行尾的中文注释保留不动
        pattern = re.compile(rf"^(\s*{toml_key}\s*=\s*)\[[^\]]*\]", re.MULTILINE)

        def repl(m, _left=left, _top=top, _w=w, _h=h):
            return f"{m.group(1)}[{_left}, {_top}, {_w}, {_h}]"

        new_text, n = pattern.subn(repl, text)
        if n == 0:
            print(f"[WARN] 未在 {path} 中找到 {toml_key}，未写入")
            continue
        text = new_text
        updated.append(toml_key)

    if updated:
        try:
            with open(path, "w", encoding="utf-8") as f:
                f.write(text)
        except OSError as e:
            print(f"[WARN] 写入 {path} 失败（{e}）")
            return []
        print(f"[INFO] 已把坐标写入 {path}: {', '.join(updated)}")
    return updated


# ---------------------------------------------------------------------- #
#  流程编排
# ---------------------------------------------------------------------- #

def select_regions(full_img, keys):
    """框选入口：优先 GUI，无 GUI 或框选为空时回退到手动输入坐标。

    Args:
        full_img: 全屏 BGR 图
        keys: 要框选的校准 key 列表

    Returns:
        dict: {key: (left, top, w, h)}
    """
    regions = try_gui_select(full_img, keys)
    if regions is None:
        regions = manual_select(full_img, keys)
    return regions


def format_region(region):
    """把区域格式化成 (left, top, w, h) 便于打印。"""
    return f"({region[0]}, {region[1]}, {region[2]}, {region[3]})"
