...
负责"持续按住方向键"的移动决策：
- 无怪物时：玩家框触达检测框最左/最右禁止区域边界 → 立即反向
  （keep_centered，五分段寻路的禁区判定部分；有怪物时攻击/追怪优先，不调用）
- 怪物在攻击范围外 → 按住方向键朝怪物靠近（hold_toward）
- 无怪物时 → 沿持久方向移动（idle_wander），触达禁止区域才反向
...
class MonsterTracker:
    ...
    def keep_centered(self, now, px, pw, left_boundary, right_boundary):
        ...
        区域；中间 3/5 沿持久方向巡逻移动。用玩家框边界而非中心判定，使
        玩家框不进入禁止区域（仅可触碰其内侧边界）。本方法是五分段寻路的禁区
        判定部分，仅在无怪物时由 GameBot 调用（有怪物时攻击/追怪优先）；触达
        禁止区域立即反向，并同步更新持久方向 _phase_dir，使玩家离开禁止区域后
        继续沿反向巡逻。
        ...
        Returns:
            ...
            （本帧不再做巡逻移动）；False 表示玩家框在中间块内，未接管。
        """
        box_left = px - pw / 2
        box_right = px + pw / 2
