...
- Per-frame logic: five-zone patrol forbidden-zone check runs first (highest priority) — the detect region is split into 5 equal horizontal zones, zones 1 & 5 are forbidden; when the player box edge touches a forbidden-zone boundary, immediately reverse. Then monsters sorted by X distance → if any `in_range` (X distance ≤ `ATTACK_DISTANCE_THRESHOLD`), face it (compare monster cx to player px) and attack (Ctrl); else **hold** a direction key toward it; else pick up (Z) every ~2s or patrol along the persistent `_phase_dir` direction (keep the exit-facing after chasing; reverse only at the forbidden-zone boundary or on stuck). `MonsterTracker` owns the stuck-detection state machine (`check_stuck_and_reverse`) — do not reintroduce a per-frame baseline refresh (that was a fixed bug). `MonsterTracker` must not mirror the held-key state; it reads `keys.held_move_key` only.
