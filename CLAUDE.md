...
| `core/screencap.py` | `ScreenCapture` — mss wrapper (region grab, idempotent `close()`). |
...
| `core/monster_detector.py` | `MonsterDetector` — detection facade (strip capture, purple ROI, annotate, calibrate, standalone CLI `main()`). |
...
- Hardcoded screen geometry: tuned for a 3440×1440 display with the game in the right half. `MonsterDetector` captures a narrow horizontal strip around the player's Y, detects the player across the full strip width (`PlayerDetector`, only a small edge margin `PLAYER_DETECT_X_SHRINK`), then matches monsters only in the **purple ROI** (player Y ± `ATTACK_RADIUS`), with a fallback to the full strip for oversized templates; `y_offset` reconciles ROI-local coords back to strip coords. A monster is `in_range` when `abs(cx - pcx) <= ATTACK_DISTANCE_THRESHOLD` (`core/attack_distance.py`). Annotated `detect_live.png` uses color-coded boxes: green=player, red cross=player center, purple=monitor range, red=monster, yellow=in-range.
...
- Per-frame logic: monsters sorted by distance → if any `in_range`, face it (compare monster cx to player px + `ATTACK_CENTER_OFFSET`) and attack (Ctrl); else **hold** a direction key toward it; else pick up (Z) every ~2s or wander by holding a direction key with random direction switches. `MonsterTracker` owns the stuck-detection state machine (`check_stuck_and_reverse`) — do not reintroduce a per-frame baseline refresh (that was a fixed bug). `MonsterTracker` must not mirror the held-key state; it reads `keys.held_move_key` only.
- **Horizontal move-range limit**: only for the monster categories listed in `[movement] move_limit_enabled_categories` (e.g. `--monster lvmogu`). `MonsterDetector` sets `move_limit_enabled` from the selected category; each loop iteration `GameBot._enforce_move_limit()` (guarded by `player_known`) calls `MonsterTracker.keep_in_range()`, which anchors to the player X on first call and reverses direction whenever the player walks more than `move_limit_pixels` to either side (single-side limit; total band = 2×), using the same `_stuck_reverse_until` protection as stuck-detection. `_range_anchor_x` is reset by `tracker.reset()` on stop.
